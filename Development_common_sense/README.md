...
`REST`는 하나의 아키텍처로 볼 수 있다. 좀 더 정확한 표현으로 말하자면, REST 는 `Resource Oriented Architecture` 이다. API 설계의 중심에 자원(Resource)이 있고 HTTP Method 를 통해 자원을 처리하도록 설계하는 것이다.

### REST 6 가지 원칙

* Uniform Interface (일관된 인터페이스)
  - 리소스에 접근하는 방식이 일관성있게 유지
  - URI로 자원을 식별하고, HTTP 메서드를 사용하여 리소스를 처리 
* Stateless (무상태성)
  - 서버는 클라이언트의 상태를 유지 X
  - 모든 요청은 필요한 정보를 자체적으로 포함해야 함
* Caching (캐싱 가능성)
  - 클라이언트와 서버 간의 상호작용에서 캐싱을 허용함으로써 성능 향상
* Client-Server (클라이언트-서버 구조)
  - 클라이언트와 서버의 역할을 명확히 분리
  - 클라이언트는 사용자 인터페이스(UI)를 담당, 서버는 데이터 저장 및 처리 로직을 담당
* Hierarchical system (계층형 시스템)
  - 중간에 프록시 서버, 로드 밸런서, 게이트웨이 등의 중간 계층이 들어가더라도 클라이언트는 이를 인지하지 못한 채 서버와 상호작용
  - 보안, 성능 향상, 로드 밸런싱 등을 위해 필요하며, 클라이언트와 서버 간의 결합도를 줄여 더 유연한 시스템을 구현 가능
* Code on demand (요청 시 코드 실행)
  - 서버가 클라이언트에 코드를 전송하고, 클라이언트가 그 코드를 실행할 수 있도록 허용하는 것을 의미
  - 클라이언트는 서버로부터 자바스크립트나 플러그인을 받아서 실행 가능 (클라이언트의 기능을 동적으로 확장)
  - 자주 사용되지는 않지만, 필요에 따라 클라이언트 기능을 확장할 수 있는 유용한 기능
...
* 행위는 `HTTP Method`로 표현하고, `GET(조회)`, `POST(생성)`, `PUT(전체 데이터 업데이트)`, `PATCH(부분적 수정)`, `DELETE(삭제)`을 분명한 목적으로 사용한다.
...
Test-Driven Development(TDD)는 매우 짧은 개발 사이클의 반복에 의존하는 소프트웨어 개발 프로세스<br>
개발자는 요구되는 새로운 기능에 대한 자동화된 테스트케이스를 작성하고 해당 테스트를 통과하는 가장 간단한 코드를 작성<br> 
...
- 개발자의 요구사항에 대한 이해 향상
  - 새로운 기능을 추가하기 전 테스트 먼저 작성
  - 해당 기능의 요구사항과 명세를 분명히 이해해야함
  - 사용자 케이스와 사용자 스토리 등을 이해하며 코드 작성 전 요구사항에 집중할 수 있도록 도움
- 새로운 기능 추가 시 기존 기능의 작동 보장
  - 새로운 기능 추가 시 기존 작동되던 기능이 작동하지 않는 상황 방지
  - 새로운 기능을 추가하는 동시에 기존의 테스트를 통해 동작 확인
- 리팩토링의 효율성
  - 코드 작성 시 컨벤션에 맞추어 일관성에 있게 작성되지만, 비즈니스 로직에 대한 고려로 코드가 더렵혀지기 마련
  - 리팩토링 진행 시 테스트코드를 돌려보며 함수를 나누는 과정을 안심하고 진행 가능 <br> 리팩토링 속도도 빨라지고 코드의 퀄리티도 향상된다.
  - 보다 객체지향적이고 확장 가능이 용이한 코드, 재설계의 시간을 단축시킬 수 있는 코드, 디버깅 시간이 단축되는 코드가 TDD 와 함께 탄생
...
TDD는 코드의 동작을 검증하는 데 초점을 맞추고, BDD는 사용자 요구사항과 비즈니스 목표에 맞춘 기능 검증에 중점
