...
데이터베이스 또는 시스템에서 한 번에 처리되어야 할 일련의 작업 단위 <br>
- 트랜잭션은 작업의 **완전성** 을 보장
- 논리적인 작업 셋을 모두 완벽하게 처리하거나 또는 처리하지 못할 경우에는 원 상태로 복구해서 작업의 일부만 적용되지 않게 함
...
트랜잭션의 모든 작업이 전부 완료되거나, 그렇지 않으면 전혀 실행되지 않음을 보장 <br>
즉, 트랜잭션 내의 작업이 중간에 실패하면 이전에 수행된 작업도 모두 롤백(rollback)

#### 일관성(Consistency)

트랜잭션이 완료된 후에도 데이터베이스는 항상 일관성 있는 상태를 유지 <br>
트랜잭션이 성공적으로 완료되면 데이터베이스의 제약 조건(무결성 제약 조건 등)이 만족

#### 고립성(Isolation)

트랜잭션이 실행되는 동안 다른 트랜잭션이 그 중간에 끼어들 수 없습니다. <br>
트랜잭션은 서로 간섭하지 않으며, 격리된 상태에서 실행 <br>
격리 수준에 따라 다른 트랜잭션의 간섭을 어느 정도 허용할지 결정 가능

#### 지속성(Durability)

트랜잭션이 성공적으로 완료되면 그 결과는 영구적으로 저장 <br>
시스템이 충돌하더라도 트랜잭션이 완료된 이후의 데이터는 손실되지 않음을 보장
...
- 트랜잭션이 처음 시작되어 아직 완료되지 않은 상태
- 이 단계에서는 트랜잭션 내에서 여러 작업(INSERT, UPDATE, DELETE 등)이 수행
= 트랜잭션은 아직 커밋(commit) 또는 롤백(rollback)되지 않았기 때문에, 트랜잭션의 결과가 데이터베이스에 반영되지 않은 상태

#### Partially Committed (부분 커밋 상태)

- 트랜잭션이 마지막 명령문을 실행한 상태로, 이제 커밋을 통해 변경 사항을 데이터베이스에 확정할 준비가 된 상태
- 이 상태에서는 아직 데이터베이스에 변경사항이 완전히 기록되지는 않았지만, 트랜잭션 내의 모든 작업이 성공적으로 완료된 상태
- 트랜잭션의 `Commit` 명령이 도착한 상태. 트랜잭션의 `commit`이전 `sql`문이 수행되고 `commit`만 남은 상태


#### Committed

- 트랜잭션이 성공적으로 완료되어, 트랜잭션에서 수행한 모든 변경 사항이 데이터베이스에 영구적으로 반영된 상태
- 커밋된 후에는 트랜잭션 내에서 수행된 작업을 되돌릴 수 없습니다. 따라서 이 상태에서는 트랜잭션이 성공적으로 끝났다고 간주됩니다.

#### Failed

- 트랜잭션이 비정상적으로 종료된 상태 
- 트랜잭션 실행 중 오류가 발생하거나, 데이터베이스가 충돌하거나, 시스템 장애가 발생한 경우 트랜잭션이 실패할 수 있음
- 실패한 트랜잭션은 더 이상 진행되지 않으며, 이때 트랜잭션은 커밋되지 않고 롤백될 수 있음


#### Aborted (중단 상태)

- 트랜잭션이 실패하거나, 트랜잭션 내에서 오류가 발생하여 모든 변경 사항이 데이터베이스에서 **롤백(rollback)**된 상태
- 트랜잭션이 중단되면, 트랜잭션 내에서 수행된 모든 작업은 취소되고, 데이터베이스는 트랜잭션이 시작되기 이전 상태로 복구
- 중단된 트랜잭션은 다시 **재시작(restart)**할 수 있습니다.
